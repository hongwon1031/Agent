"""
Prototype 5 Agent: Reflection-Based Prompt Tuning System

고정 워크플로우 + Reflection 기반 Instruction 튜닝
"""

import json
from typing import Dict, Any, List

from core.reflection_planner import ReflectionPlanner  # NEW: ReflectionPlanner
from core.llm_validator import LLMValidator
from tools.hybrid_tools import (
    RuleExtractTool, LLMExtractTool, DefinitionExtractToolV2,
    RuleCartesianTool, LLMCartesianTool,
    SectionClassifierTool
)


class Prototype5Agent:
    """
    Reflection-Based Prompt Tuning System

    워크플로우:
        1. DocumentAccessor: 섹션 추출
        2. ReflectionPlanner: 고정 워크플로우 로드
        3. Tool Executor: Task 실행 (instruction 포함)
        4. LLM Validator: 결과 검증
        5. Reflection & Tuning: 실패 시 instruction 누적
        6. 최종 결과 반환
    """
    ...
    def __init__(self, max_replan_per_task: int = 6):
        ...
        """
        self.planner = ReflectionPlanner()  # NEW: ReflectionPlanner
    ...
    def run(self, doc: Any) -> Dict[str, Any]:
        ...
        """
        print("="*80)
        print("Prototype 5: Reflection-Based Prompt Tuning System")
        print("="*80)
        ...
        # Step 2: 계획 수립 (고정 워크플로우 로드)
        print("\n[STEP 2] Loading fixed workflow...")
        ...
        for attempt_num in range(self.max_replan + 1):
            ...
            if not result.success:
                ...
                if attempt_num < self.max_replan:
                    new_plan = self.planner.reflect_and_tune(
                    ...
                    if "error" not in new_plan:
                        ...
                        print(f"  → Reflected: {new_plan.get('reasoning')}")
                        continue
