...
def attach_reflection_summary(result: dict) -> dict:
    """execution_log를 요약한 reflection_summary를 result에 추가"""
    summary = []
    ...
    for log_entry in result.get("execution_log", []):
        ...
        for i, attempt in enumerate(attempts, 1):
            ...
            task_summary["attempts"].append({
                "attempt": i,
                "tool": attempt.get("tool"),
                "success": attempt.get("success"),
                "instruction_used": bool(params.get("instruction")),
            })

        summary.append(task_summary)

    result["reflection_summary"] = summary
    return result
...
def main():
    ...
    if input_path.is_dir():
        ...
        for doc_path in json_files:
            ...
            attach_reflection_summary(result)
