  ...
  "value": <정수>
    ...
    def execute(self, doc: Any, params: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # Handle both dict and list formats for grouping_logic
            if isinstance(grouping_logic, list):
                # Convert list format to dict format
                groups = grouping_logic
            elif isinstance(grouping_logic, dict):
                if "groups" not in grouping_logic:
                    return ToolResult(
                        success=False,
                        error="Invalid grouping_logic dict (missing 'groups' key)",
                        tool_name=self.name
                    )
                ...
                return ToolResult(
                    success=False,
                    error=f"Invalid grouping_logic type: {type(grouping_logic)}",
                    tool_name=self.name
                )
            ...
            if not groups:
                return ToolResult(
                    success=False,
                    error="grouping_logic has no groups",
                    tool_name=self.name
                )

            print(f"\n[GENERATE] Generating final combinations...")

            # Normalize grouping_logic to dict format for _generate_combinations
            if isinstance(grouping_logic, list):
                normalized_grouping = {"groups": groups, "unmatched": {"definition_indices": []}}
            else:
                normalized_grouping = grouping_logic
            ...
            # Calculate stats (use already extracted groups variable)
            matched_def_count = sum(len(g.get("definition_indices", [])) for g in groups)
            unmatched_def_count = len(normalized_grouping.get("unmatched", {}).get("definition_indices", []))
