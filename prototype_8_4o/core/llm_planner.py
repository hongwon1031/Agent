...
from tools.tool_schemas import get_schema_prompt
...
class LLMPlanner:
    ...
    def generate_next_task(
        ...
        # task_type -> 기본 tool 매핑
        type_to_tool = {
            "classify_sections": "section_classifier",
            "extract_definitions": "definition_extract_v2",
            "table_split": "llm_table_split",
            "condition_extraction": "condition_extract",
            "condition_transform": "condition_transform",
            "grouping": "grouping_logic_extractor",
            "combination_generation": "combination_generator",
        }
        ...
        try:
            ...
            stats = self._llm_stats["generate_next_task"]
            stats["call_count"] += 1
            ...
            if action == "end":
                ...
                if 0 <= success_count < len(sequence):
                    expected_type = sequence[success_count]
                    actual_type = task.get("task_type")
                    if actual_type != expected_type:
                        print(
                            f"[LLMPlanner] Fix task_type: {actual_type} "
                            f"-> {expected_type} (success_count={success_count})"
                        )
                        task["task_type"] = expected_type

                    # tool_name도 일관되게 맞춰줌
                    expected_tool = type_to_tool[expected_type]
                    if task.get("tool_name") != expected_tool:
                        print(
                            f"[LLMPlanner] Fix tool_name: {task.get('tool_name')} "
                            f"-> {expected_tool}"
                        )
                        task["tool_name"] = expected_tool
