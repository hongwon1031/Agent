...
2. **가입나이 파싱** (최소/최대 연령 분리):
   - **수식(min, max 등) 있으면** → 수식 문자열 그대로 보존 (계산 금지)
   - **수식 없으면** → 숫자 추출
   - 예시:
     * `"만15세 ~ 70세"` → 최소: `"만 15세"`, 최대: `"70세"`
     * `"만15세 ~ min[80-년납, 70]세"` → 최소: `"만 15세"`, 최대: `"min[80-년납, 70]세"`
     * `"max[15, 20]세 ~ 70세"` → 최소: `"max[15, 20]세"`, 최대: `"70세"`
...
1. **필수 스키마 존재**:
   ...
   - "보험기간" (list 또는 str)
   - "납입기간" (list 또는 str)
   - "주피보험자최소가입연령" (str: 수식 문자열 또는 숫자/나이 문자열)
   - "주피보험자최대가입연령" (str: 수식 문자열 또는 숫자/나이 문자열)
...
3. **가입연령 형식** (최소/최대 동일 규칙):
   - **원본에 수식(min/max 등) 있으면** → 변환 결과도 수식 문자열
   - **원본에 수식 없으면** → 변환 결과는 숫자 문자열
   - ⚠️ **검증 방법**: 원본 데이터 확인 필수!
...
3. **🔴 수식 처리 오류** 🔴:
   - **규칙**: 원본에 수식(min/max 등) 있으면 → 결과도 수식 보존, 원본에 수식 없으면 → 결과는 숫자
   - **에러 케이스:**
     * 원본 `"min[80-년납, 70]세"` → 결과 `"70세"` ❌ (수식을 계산함)
     * 원본 `"만15세 ~ 70세"` → 결과 `"min[...]"` ❌ (수식 추가함)
   - **올바른 예:**
     * 원본 `"만15세 ~ 70세"` → 결과 최대 `"70세"` ✅
     * 원본 `"min[80-년납, 70]세"` → 결과 최대 `"min[80-년납, 70]세"` ✅
   - **이 오류 발견시**: suggestion에 원본의 정확한 값을 명시하세요!
...
1. **먼저 원본 데이터를 확인하세요!**
   - 변환 결과가 올바른지 판단하기 **전에** 원본 데이터를 봐야 합니다
   - 예: 최대가입연령이 "70세"일 때, 원본을 보고 수식이 있었는지 확인

2. **수식 처리 검증 방법**:
   - 원본 가입나이 컬럼에서 **min[...]이나 max[...] 같은 수식이 있는지** 찾기
   - **수식이 원본에 없으면** → 변환 결과 "70세"는 정상 ✅
   - **수식이 원본에 있으면** → 변환 결과도 수식이어야 함 ✅

3. **판단 기준**:
   - ❌ 잘못: "70세"를 보고 무조건 "숫자로 계산되었다" 판단
   - ✅ 올바름: 원본이 "만15세 ~ 70세"면 → 결과 "70세" 정상
   - ✅ 올바름: 원본이 "min[80-년납, 70]세"면 → 결과도 "min[80-년납, 70]세"여야 함

**중요한 마음가짐**:
- **명백한 오류**만 지적하세요
- 원본 확인 없이 판단하지 마세요!

**Suggestion 작성 가이드 (매우 중요)**:
수식 계산 오류를 발견했다면, suggestion에 다음을 포함하세요:

1. 원본 데이터에서 정확한 수식 찾기
2. "**반드시** 이 정확한 값을 사용하세요: 'min[80-납입기간, 70]세'" 형태로 명령
3. "계산하지 말고 원문 그대로 복사하세요" 명시

예시:
- ❌ 나쁜 suggestion: "최대가입연령을 수식 문자열로 보존하세요"
- ✅ 좋은 suggestion: "원본 가입나이_남 컬럼의 'min[80-납입기간, 70]세'를 주피보험자최대가입연령에 **그대로** 복사하세요. 숫자 70으로 계산하지 마세요."
...
{{
  ...
  "suggestions": [
    // errors가 있을 때만 구체적이고 실행 가능한 개선 제안
    // ❌ 일반적: "최대가입연령을 수식 문자열로 보존하세요"
    // ✅ 구체적: "원본 데이터의 'min[세만기-년납, 70] 세'를 그대로 복사하세요. 70으로 계산하지 마세요."
    // ✅ 구체적: "주피보험자최대가입연령 필드에 원본의 정확한 수식을 넣으세요: 'min[80-납입기간, 70]세'"
    // 원본 데이터에서 실제 값을 찾아서 구체적으로 제안하세요!
  ],
  "reasoning": "ConditionTransform이 스키마 매핑, 리스트 분리, 수식 보존을 올바르게 수행했는지 평가"
}}

**참고**: 수식 보존은 중요하지만 **원본 데이터에 수식이 존재한 경우에만** 적용됩니다.
- 원본 최대가입연령에 `min[...]`, `max[...]` 같은 수식이 있으면 → 변환 결과도 동일한 수식 문자열이어야 합니다.
- 원본 최대가입연령에 수식이 없고 단순 상한(예: `70세`)만 있으면 → 변환 결과가 `70세`인 것은 정상이며 is_valid=true입니다.
- 결과 값만 보고 판단하지 말고 **반드시 원본 가입나이 컬럼을 먼저 확인하세요**.
"""
...
# ================================================주석 처리================================================
# ================================================================================================
def build_validate_condition_extract_llm_prompt_1(
    condition_sections: List[Dict[str, Any]],
    extracted_header: List[str],
    extracted_data: List[List[str]],
    definition_sections: List[Dict[str, Any]] = None,
...
def build_llm_merge_prompt_1(content_str: str, instruction: str = "") -> str:
    base = f"""
    당신은 테이블과 텍스트 주석을 병합하는 전문가입니다.
...
def build_intelligent_condition_extract_prompt_1(
    condition_sections: List[Dict[str, Any]],
    definition_sections: List[Dict[str, Any]] = None,
    instruction: str = ""
) -> str:
...
def build_condition_transform_prompt(
    ...
    if instruction:
        extra_instruction = f"""
🚨🚨🚨 **CRITICAL OVERRIDE - 이전 실패 방지를 위한 필수 지시사항** 🚨🚨🚨

{instruction}

❗ 위 지시사항은 절대적이며, 아래 모든 규칙보다 우선합니다!
특히 이전 실행에서 실패한 부분이므로 반드시 수정하세요!
...
**🔴 핵심 규칙 🔴**:
- **수식(min[], max[] 등) 있으면** → 수식 문자열 그대로 보존 (계산 절대 금지!)
- **수식 없으면** → 숫자 추출
- "만" 있으면 공백 정규화 ("만15세" → "만 15세")

**예시**:
- 입력: `"만15세 ~ 70세"` → 최소: `"만 15세"`, 최대: `"70세"` ✅
- 입력: `"만15세 ~ min[80-년납, 70]세"` → 최소: `"만 15세"`, 최대: `"min[80-년납, 70]세"` ✅
- 입력: `"max[15, 20]세 ~ 70세"` → 최소: `"max[15, 20]세"`, 최대: `"70세"` ✅
- ❌ 잘못된 예: 입력 `"min[80-년납, 70]세"` → 출력 `"70세"` (수식 계산함)
...
def build_next_task_prompt(
    ...
    if instruction:
        extra_instruction = (
            f"""\n**추가 지시사항 (반드시 반영)**:\n{instruction}\n\n---\n\n
            추가 지시사항을 task.parameters.instruction 필드로 반드시 넣어라"""
        )
...
- last_validation_feedback에 errors/suggestions가 있다면,
  해당 task를 재실행할 때 반드시 `task.parameters.instruction` 필드에
  그 내용을 요약해서 넣어라.
...
{{
  ...
  "task": {{
    ...
    "parameters": {{
      "sections": "$sections",
      "condition_indices": "{{task0.data.condition}}",
      "instruction": "이전 실패 원인: ... / 개선 지시: ... 컬럼들을 반드시 포함해라"
  }},
    ...
  }}
}}
...
7. task 순서는 고정되어야 합니다 (backtracking/재시도 제외).
