...
def execute_task_node(state: AgentState) -> Dict[str, Any]:
    ...
    # ========== STEP 2.5: Inject backtracking instruction into params ==========
    # CRITICAL: Pass backtrack_reasoning to tools so they can adjust their behavior
    backtrack_reasoning = state.get("backtrack_reasoning", "")
    if backtrack_reasoning:
        # Inject instruction into resolved_params if not already present
        if "instruction" not in resolved_params:
            resolved_params["instruction"] = backtrack_reasoning
            print(f"[INJECT] Added backtrack instruction to params: {backtrack_reasoning[:100]}...")
        else:
            # Append to existing instruction
            existing = resolved_params["instruction"]
            resolved_params["instruction"] = f"{existing}\n\n[BACKTRACK CONTEXT]: {backtrack_reasoning}"
            print(f"[INJECT] Appended backtrack instruction to existing params instruction")
