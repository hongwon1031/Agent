...
TOOL_SCHEMAS = {
    "section_classifier": {
        "description": "Domain-agnostic LLM-based section classifier (RECOMMENDED). Classifies sections into semantic categories based on instruction.",
        ...
        "parameters": {
            ...
            "instruction": {
                "type": "string",
                "description": "Domain-specific classification instruction with category definitions and criteria",
                "required": True,
                "example": """This is an insurance document. Classify sections into:
- definition_core: Main product definition table
- definition_annotation: Annotations about definitions
- condition: Coverage conditions
- other: All other sections
Output JSON: {"category_name": [section_indices], "reasoning": "..."}"""
            }
        },
        "returns": {
            "Categories defined in instruction (e.g., definition_core)": "list[int] - Section indices for this category",
    ...
    "llm_extract": {
        "description": "Domain-agnostic LLM-based data extraction. Extracts structured data from content based on instruction.",
        "supported_formats": ["table", "text", "mixed"],
        "parameters": {
            "content": {
                "type": "dict or string",
            ...
            "instruction": {
                "type": "string",
                "description": "Domain-specific extraction instruction with field names and processing rules",
                "required": True,
                "example": """Extract product information with these fields:
- Product name (column: 명칭)
- Product type (column: 유형)
- Coverage category (column: 보장종목)

Rules:
- Skip annotation rows marked with ※, 주:, *
- Normalize whitespace
Output: {"header": [...], "data": [[...]], "extraction_method": "table"}"""
            }
        },
        "returns": {
    ...
    "llm_cartesian": {
        "description": "Domain-agnostic LLM-based combination generation. Parses multi-value cells and generates cartesian product based on instruction.",
        "parameters": {
            "header": {
                "type": "list[string]",
                "description": "Column names",
            ...
            "instruction": {
                "type": "string",
                "description": "Domain-specific transformation instruction with delimiter rules and combination logic",
                "required": True,
                "example": """Parse multi-value cells and generate combinations:

**Column-wise Delimiter Detection**:
- Column 1 (product_name): Primary delimiter is / (don't split on commas)
- Column 2 (product_type): Primary delimiter is , (don't split on /)
- Column 3 (coverage): Primary delimiter is /

**Rules**:
1. Count delimiter frequency per column before splitting
2. Use only the most frequent delimiter per column
3. Strip whitespace from split values
4. Generate cartesian product across columns
5. Create one definition per combination
