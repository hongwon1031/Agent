...
def plan_node(state: AgentState) -> Dict[str, Any]:
    ...
    if tool_name in INSTRUCTION_AWARE_TOOLS:
        if extra_instruction:
            # 재시도/백트래킹 시: validator feedback을 instruction으로 강제 전달
            params["instruction"] = extra_instruction
        else:
            # 첫 실행 시: 플래너가 멋대로 넣은 instruction은 제거
...
def validate_task_node(state: AgentState) -> Dict[str, Any]:
    ...
    # CRITICAL: Normalize task_type using same aliases as LLMValidator
    TASK_TYPE_ALIASES = {
    ...
    normalized_task_type = TASK_TYPE_ALIASES.get(task_type, task_type)
    print(f"[INFO] Normalized task_type: {task_type} -> {normalized_task_type}")

    # Build context with previous results
    context = {"previous_results": state.get('task_results', [])}

    # NEW: For extract (definition_extract_v2) validation, add core/annotation sections
    if normalized_task_type == "extract":
        task_results = state.get('task_results', [])
        all_sections = state.get("sections", [])

        # section_classifier 결과에서 definition indices 추출 (tool_used 기반)
        definition_core_indices = []
        definition_annotation_indices = []
        for result in task_results:
            if result.get("success") and result.get("tool_used") == "section_classifier":
                data = result.get("data", {})
                definition_core_indices = data.get("definition_core", [])
                definition_annotation_indices = data.get("definition_annotation", [])
                break

        # Build core_sections and annotation_sections from all_sections
        if all_sections:
            if definition_core_indices:
                core_sections = [s for s in all_sections if s.get("index") in definition_core_indices]
                context["core_sections"] = core_sections

            if definition_annotation_indices:
                annotation_sections = [s for s in all_sections if s.get("index") in definition_annotation_indices]
                context["annotation_sections"] = annotation_sections

        print(f"[CONTEXT] Definition extract validation context prepared:")
        print(f"  - all_sections count: {len(all_sections)}")
        print(f"  - definition_core_indices: {definition_core_indices}")
        print(f"  - definition_annotation_indices: {definition_annotation_indices}")
        print(f"  - core_sections count: {len(context.get('core_sections', []))}")
        print(f"  - annotation_sections count: {len(context.get('annotation_sections', []))}")

    # NEW: For table_split validation, add original definition data
    if normalized_task_type == "table_split":
        task_results = state.get('task_results', [])

        # definition_extract_v2 결과에서 원본 데이터 추출 (tool_used 기반)
        for result in task_results:
            if result.get("success") and result.get("tool_used") == "definition_extract_v2":
                data = result.get("data", {})
    ...
    if normalized_task_type == "condition_transform":
        task_results = state.get('task_results', [])

        # condition_extract 결과에서 원본 데이터 추출 (tool_used 기반)
        for result in task_results:
            if result.get("success") and result.get("tool_used") == "condition_extract":
                data = result.get("data", {})
                context["original_header"] = data.get("header", [])
                context["original_data"] = data.get("data", [])
                break
    ...
    if normalized_task_type == "extract_condition":
        task_results = state.get('task_results', [])

        # Get all_sections from state
        all_sections = state.get("sections", [])

        # section_classifier 결과에서 condition indices 추출 (tool_used 기반)
        condition_indices = []
        for result in task_results:
            if result.get("success") and result.get("tool_used") == "section_classifier":
                data = result.get("data", {})
    ...
    if normalized_task_type == "grouping":
        task_results = state.get("task_results", [])
    ...
    # NEW: For classify (section_classifier) validation, add all_sections
    if normalized_task_type == "classify" or normalized_task_type == "search":
        all_sections = state.get("sections", [])
        if all_sections:
            context["all_sections"] = all_sections

        print(f"[CONTEXT] Section classifier validation context prepared:")
        print(f"  - all_sections count: {len(all_sections)}")

    # NEW: For transform (cartesian) validation, add extracted_data
    if normalized_task_type == "transform":
        task_results = state.get('task_results', [])

        # Find the most recent definition extraction result
        # (could be from table_split or definition_extract_v2)
        for result in reversed(task_results):
            if result.get("success"):
                tool_used = result.get("tool_used", "")
                if tool_used in ["llm_table_split", "rule_table_split", "definition_extract_v2"]:
                    data = result.get("data", {})
        ...
        print(f"[CONTEXT] Transform validation context prepared:")
        if "extracted_data" in context:
            print(f"  - extracted_data header: {context['extracted_data'].get('header', 'NOT FOUND')}")
            print(f"  - extracted_data rows: {len(context['extracted_data'].get('data', []))}")
        else:
            print(f"  - extracted_data: NOT FOUND")

    # NEW: For generate (final combinations) validation, add grouping_logic
    if normalized_task_type == "generate":
        task_results = state.get('task_results', [])

        # Find grouping_logic_extractor result
        for result in reversed(task_results):
            if result.get("success") and result.get("tool_used") == "grouping_logic_extractor":
                context["grouping_logic"] = result.get("data", {})
                break

        print(f"[CONTEXT] Generate validation context prepared:")
        if "grouping_logic" in context:
            print(f"  - grouping_logic groups count: {len(context['grouping_logic'].get('groups', []))}")
        else:
            print(f"  - grouping_logic: NOT FOUND")

    # Call validator with original task_type (validator will normalize it internally)
