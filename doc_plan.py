...
def build_prompt(raw_json: str) -> str:
    """document_analysis + plan을 한 번에 생성하는 프롬프트 (통합 버전)"""
    tool_lines = "\n".join(
    ...
    # f-string 내부에서 {{ }}는 리터럴 중괄호로 처리됨
    prompt_template = """
        너는 Document Analyzer + Planner Agent이다.
        원본 JSON 문서를 받아서:
        1. 문서 구조를 분석하고 (document_analysis)
        2. 가입가능조건 JSON을 생성하기 위한 실행 계획(plan)을 수립하라.
        ...
        ⚠️⚠️⚠️ PART 1: Document Analysis (문서 구조 분석) ⚠️⚠️⚠️

        [1-1. definition_section 분석]
        ...
        ⚠️⚠️⚠️ PART 2: Planning (실행 계획 수립) ⚠️⚠️⚠️

        [2-1단계: 체크리스트 검토]
        다음 항목들을 순서대로 확인하고, reasoning.checklist에 분석 결과를 작성하라:

        □ [언어]
          - document_analysis의 definition_section.title, definition_section.columns,
        ...
        [너의 역할]
        1. 문서 구조를 파악하여 document_analysis 생성
        2. document_analysis를 기반으로 reasoning + plan 생성
        - 출력: document_analysis + reasoning + plan (가입가능조건 JSON이 아님!)
        ...
        위 문서를 분석하여 document_analysis, reasoning, plan을 모두 생성하라.
        한국어로 작성하라.
        """
    ...
    for path in targets:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Document Analyzer + Planner Agent (통합). Chain-of-Thought 방식으로 문서 구조를 분석한 뒤, 그 결과를 바탕으로 사고 과정(reasoning)과 실행 계획(plan)을 수립한다. 체크리스트를 꼼꼼히 확인하고, 대안을 검토하며, 각 선택의 근거를 명확히 한다.",
                },
        ...
        print(f"\n=== Generated Document Analysis + Plan ===")
        print(result_text)
        ...
        # 결과 저장 (doc_plan/ 디렉토리)
        result_dir = path.parent / "doc_plan"
        result_dir.mkdir(exist_ok=True)

        out_path = result_dir / f"{path.stem}_doc_plan.json"
        ...
        try:
            ...
            # OnlyDocOutput 스키마 검증 (document_analysis 부분만)
            if "document_analysis" in result_obj:
                try:
                    OnlyDocOutput.model_validate(
                        {"document_analysis": result_obj["document_analysis"]}
                    )
                    print(f"\n✅ document_analysis 스키마 검증 성공")
                except ValidationError as e:
                    print(f"\n⚠️  document_analysis 스키마 검증 실패:")
                    try:
            ...
            print(f"\n✅ Document Analysis + Plan 저장 성공: {out_path}")
