...
TOOL_SCHEMAS = {
    "section_classifier": {
        ...
        "parameters": {
            ...
            "instruction": {
                "type": "string",
                "description": "Optional. Validation feedback 기반 재시도 지시사항. 프롬프트 최상단에 그대로 포함.",
                "required": False,
                "value": "$instruction",
            },
        },
    ...
    "definition_extract_v2": {
        "description": "Extract definition table from classified sections",
        "supported_formats": ["table", "text", "mixed"],
        "parameters": {
            "sections": {
                "type": "list[dict]",
    ...
    "llm_table_split": {
        "description": "LLM-based semantic splitting of multi-value cells in Definition table",
        "supported_formats": ["table"],
        "parameters": {
            "header": {
                "type": "list[string]",
    ...
    "rule_table_split": {
        "description": "Rule-based fast table cell splitting (deterministic, fallback for llm_table_split)",
        "supported_formats": ["table"],
        "parameters": {
            "header": {
                "type": "list[string]",
            ...
            "row_indices": {
                "type": "list[int]",
                "description": "Specific row indices to process (optional)",
                "required": False,
            },
        },
        "returns": {
            "header": "list[string] - Same as input header",
            "data": "list[list[any]] - Data with cells split by delimiters (/, ·, etc.)",
            "notes": "string - Splitting notes",
        },
    },
    "condition_transform": {
        "description": "Transform condition table: split values + parse age formulas + map to standard schema",
        "supported_formats": ["table"],
        "parameters": {
            "header": {
                "type": "list[string]",
