...
def build_prompt(raw_json: str) -> str:
    """raw JSON에서 직접 plan을 생성하는 프롬프트 (document_analysis 생략)"""
    tool_lines = "\n".join(
    ...
    prompt_template = """
        ...
        출력 형식:
        {
            "reasoning": {
                "checklist": {
                    ...
                    "수식": {
                        "check": "문서에서 수식(min, max, 계산식 등) 포함 여부 확인",
                        "detected": "true / false",
                        "decision": "FormulaEvaluator 필요 / 불필요",
                        "reason": "상세한 판단 근거"
                    },
                    "소제목": {
                        "check": "조건 섹션에 소제목 구조 존재 여부 확인",
                        "detected": "true / false",
                        "decision": "SubtitleMapper 계열 필요 / 불필요",
                        "reason": "상세한 판단 근거"
                    },
                    "복잡표": {
                        "check": "rowspan/colspan 복잡도, 병합 셀 분석",
        ...
        [1단계: 체크리스트 검토]
        원본 JSON을 직접 분석하여 다음 항목들을 확인하고, reasoning.checklist에 분석 결과를 작성하라:

        □ [언어]
          - 문서의 elements를 순회하며 title, 표의 컬럼명 등을 확인
        ...
        □ [수식]
          - 표의 셀 내용에서 min[...], max[...], 계산식(+, -, *) 등 확인
          - 감지 결과:
            * 복잡한 수식 포함 → FormulaEvaluator 필수
            * 단순한 수식 → 선택적 사용
            * 수식 없음 → 생략

        □ [소제목]
          - 조건 섹션에서 paragraph 타입이 "title"인 항목들 확인
          - 소제목-표 구조가 반복되는지 분석
          - 감지 결과:
            * 소제목 + 표 구조 → SubtitleMapper 또는 관련 Tool 필요
            * 소제목 없음 → 생략

        □ [복잡표]
          - 표의 rowspan, colspan 속성 확인
          - 병합 셀의 복잡도 분석
          - 감지 결과:
            * 복잡한 병합 → RowspanHandler / ColspanHandler / ComplexTableParser 고려
            * 단순 구조 → 생략
        ...
        위 원본 문서를 직접 분석하여 reasoning과 plan을 생성하라.
        한국어로 작성하라.
        """
    ...
    for path in targets:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Direct Planner Agent. 문서 구조 분석 없이 원본 JSON을 직접 받아 Chain-of-Thought 방식으로 사고 과정(reasoning)을 먼저 상세히 작성한 뒤, 그 결과를 바탕으로 실행 계획(plan)을 수립한다. 체크리스트를 꼼꼼히 확인하고, 대안을 검토하며, 각 선택의 근거를 명확히 한다.",
                },
