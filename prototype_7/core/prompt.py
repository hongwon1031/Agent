...
def build_dynamic_planning_prompt_v7(document_summary: str, document: str, instruction: str = "") -> str:
    """
    PROTOTYPE 7: Build prompt for dynamic task planning (3-7 tasks based on complexity).

    Args:
        document_summary: Summary of document structure
        document: Full document content (truncated if too long)
        instruction: Additional user instructions

    Returns:
        Prompt for LLM to generate dynamic execution plan
    """
...
"""
    ...
    prompt = f"""
{extra_instruction}당신은 보험 약관 문서를 분석하여 최적의 작업 실행 계획을 수립하는 전문가입니다.

## 입력 문서 요약
{document_summary}

## 입력 문서 샘플
{document[:2000]}... (총 {len(document)} 자)

## 목표
문서의 복잡도를 분석하고, **3~7개의 작업**으로 구성된 실행 계획을 생성하세요.

## 작업 유형 (사용 가능한 도구)

### 1. classify_sections (섹션 분류) - Optional
- **도구**: section_classifier
- **언제**: 문서 구조가 불명확하거나 여러 섹션이 혼재
- **출력 키**: section_info

### 2. extract_definitions (정의 추출)
- **도구**: definition_extractor (규칙 기반) / llm_extractor (LLM 기반)
- **언제**: 정의 테이블이 명확히 구분된 섹션에 있을 때
- **출력 키**: definition_core

### 3. normalize_definitions (정의 정규화)
- **도구**: definition_normalizer
- **언제**: 추출된 정의의 컬럼명을 표준화해야 할 때
- **출력 키**: normalized_definitions

### 4. extract_conditions (조건 추출)
- **도구**: condition_extractor / llm_extractor
- **언제**: 가입 조건 테이블이 별도 섹션에 있을 때
- **출력 키**: condition_core

### 5. normalize_conditions (조건 정규화)
- **도구**: condition_normalizer
- **언제**: 추출된 조건의 컬럼명을 표준화해야 할 때
- **출력 키**: normalized_conditions

### 6. extract_grouping_logic (그룹핑 로직 추출)
- **도구**: grouping_logic_extractor
- **언제**: 정의와 조건을 매칭해야 할 때
- **출력 키**: grouping_logic

### 7. generate_combinations (조합 생성)
- **도구**: combination_generator
- **언제**: 그룹핑 로직을 바탕으로 최종 조합을 만들 때
- **출력 키**: final_definitions

## 계획 수립 가이드라인

### 단순 문서 (3단계):
- **조건**: 정의만 있고 가입 조건이 없거나 단일 테이블
- **플로우**:
  1. extract_definitions
  2. normalize_definitions
  3. (final_definitions = normalized_definitions로 설정)

### 일반 문서 (5-6단계):
- **조건**: 정의와 조건이 명확히 분리, 구조 단순
- **플로우**:
  1. extract_definitions
  2. normalize_definitions
  3. extract_conditions
  4. normalize_conditions
  5. extract_grouping_logic
  6. generate_combinations

### 복잡 문서 (7단계):
- **조건**: 섹션이 불명확, 여러 상품 혼재, 복잡한 구조
- **플로우**:
  1. classify_sections (섹션 파악)
  2. extract_definitions
  3. normalize_definitions
  4. extract_conditions
  5. normalize_conditions
  6. extract_grouping_logic
  7. generate_combinations
...
{{
  ...
  "tasks": [
    {{
      ...
      "description": "5-12페이지에서 정의 테이블 추출",
      "tool_name": "definition_extractor",
      "fallback_tool": "llm_extractor",
      "parameters": {{
        "document": "{{"{{document}}"}}"
        "pages": "5-12"
      }},
      "dependencies": [],
      "output_key": "definition_core"
    }},
    {{
      "task_id": "task1",
      "task_type": "normalize_definitions",
      "description": "정의 테이블의 컬럼명 정규화",
      "tool_name": "definition_normalizer",
      "fallback_tool": null,
      "parameters": {{
        "header": "{{"{{task0.definition_core.header}}"}}"
        "data": "{{"{{task0.definition_core.data}}"}}",
      }},
      "dependencies": ["task0"],
      "output_key": "normalized_definitions"
    }},
    ... (3-5 more tasks)
  ],
  "reasoning": "이 문서는 정의와 조건이 명확히 분리되어 있고 구조가 단순하므로 5단계 플로우가 적합합니다."
}}

## 중요 규칙

1. **의존성 순서**: dependencies는 반드시 이전 task_id만 참조
2. **템플릿 문법**: 파라미터에서 이전 결과 참조 시 "{{"{{taskN.output_key.field}}"}}\" 사용
3. **Fallback 전략**: 규칙 기반 도구의 fallback은 LLM 도구
4. **최소 단계**: 불필요한 작업은 제외 (단순 문서에 섹션 분류 X)
5. **output_key 일관성**: 다음 작업의 dependencies가 참조할 키 명확히 지정

이제 위 규칙에 따라 문서를 분석하고 최적의 작업 계획을 JSON으로 반환하세요.
"""
    return prompt
