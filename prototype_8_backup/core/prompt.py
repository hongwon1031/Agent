...
def build_llm_extract_prompt(content_str: str, content_type: str, instruction: str = "") -> str:
    ...
    **[NEW] 값 그룹핑 규칙**:
    - 셀 값이 의미적으로 다른 여러 그룹으로 나뉘는 경우, 리스트로 반환하세요
    - 예: "간편심사(315)형 /간편심사(335)형 /간편심사(355)형 /일반심사형"
      → ["간편심사(315)형 /간편심사(335)형 /간편심사(355)형", "일반심사형"]
      (간편심사 계열과 일반심사를 분리)
    - 같은 의미 그룹 내에서는 슬래시(/)를 유지하세요
    - 단일 그룹이거나 분리가 불필요한 경우, 문자열로 반환하세요

    **그룹핑 판단 기준**:
    - "일반형 /간편심사형" → 의미적으로 다름 → ["일반형", "간편심사형"]
    - "간편심사(315)형 /간편심사(335)형 /간편심사(355)형" → 같은 의미 → "간편심사(315)형 /간편심사(335)형 /간편심사(355)형" (문자열)
    - "환급(A)형 /환급(B)형 /일반형" → 환급 계열 vs 일반 → ["환급(A)형 /환급(B)형", "일반형"]
    """
    ...
    {{
      ...
      "notes": "처리 내용 (리스트로 분리한 경우 설명)"
    }}

    **주의**: 셀 값은 문자열 또는 리스트일 수 있습니다. 의미 그룹이 2개 이상이면 리스트를 사용하세요.
    """
...
- "condition_transform"      # condition_extract 결과를 최종 스키마로 변환할 때 사용 (condition_transform)
