...
class Prototype3Agent:
    ...
    def run(self, doc: Any) -> Dict[str, Any]:
        ...
        for task in tasks:
            ...
            task_result = self._execute_task_with_replan(
                task, doc, sections, previous_results, execution_log
            )
    ...
    def _execute_task_with_replan(
        ...
        for attempt_num in range(self.max_replan + 1):
            ...
            if not result.success:
                ...
                # sections 정보 추가 (classify/search validation에 필요)
                if task["type"] in ["classify", "search"]:
                    validation_context["all_sections"] = sections

                # core_sections, annotation_sections 정보 추가 (extract V2 validation에 필요)
                if task["type"] == "extract":
                    depends_on = task.get("depends_on")
                    if depends_on:
                        prev_result = previous_results.get(depends_on, {})
                        if "definition_core" in prev_result:
                            core_idxs = prev_result.get("definition_core", [])
                            anno_idxs = prev_result.get("definition_annotation", [])
                            all_idxs = sorted(set(core_idxs + anno_idxs))

                            core_sections = [sections[i] for i in core_idxs if 0 <= i < len(sections)]
                            annotation_sections = [sections[i] for i in anno_idxs if 0 <= i < len(sections)]
                            definition_sections = [sections[i] for i in all_idxs if 0 <= i < len(sections)]

                            validation_context["core_sections"] = core_sections
                            validation_context["annotation_sections"] = annotation_sections
                            validation_context["definition_sections"] = definition_sections
    ...
    def _prepare_validation_context_v2(
        ...
        """
        Validation을 위한 컨텍스트 준비 (정의 섹션 정보 포함)
        """
        context: Dict[str, Any] = {}

        # Classify/Search validation: 전체 섹션 정보 필요
        if task["type"] == "classify" or task["type"] == "search":
            # SectionClassifier는 all_sections를 context로 받음
            # 현재는 task parameters에 $sections가 있지만, validation에서도 필요
            # agent.run()에서 sections를 저장해두거나, task parameters로부터 가져올 수 있음
            # 간단하게 task parameters에서 sections 참조
            params = task.get("parameters", {})
            if "sections" in params:
                # 이미 resolved된 sections가 있으면 사용
                # 하지만 여기서는 아직 resolve 전이므로, previous_results에서 찾거나
                # agent의 sections를 직접 전달해야 함
                pass
            # 더 나은 방법: agent.run()의 sections를 context로 전달
            # 이를 위해 _execute_task_with_replan에서 sections를 받아야 함
            # 현재는 간단히 패스하고, 호출 시점에 추가
            context["all_sections"] = []  # placeholder - will be filled by caller
        ...
        # Extract validation: 원본 컨텐츠가 필요 (table 또는 text)
            ...
            if depends_on:
                ...
                # V2 workflow: classifier 결과에서 core_sections 준비
                if "definition_core" in prev_result:
                    # SectionClassifier 결과 - core 섹션들의 실제 내용 필요
                    # sections는 agent.run()에서 전달받아야 함
                    context["core_sections"] = []  # placeholder - will be filled by caller
                else:
                    # V1 workflow: Search 단계에서 찾은 정의 섹션의 원본 전달
                    original_content = prev_result.get("found_content") or prev_result.get("found_table")
                    context["original_content"] = original_content
