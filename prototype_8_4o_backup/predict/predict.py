...
def load_gt_rows(gt_path: Path) -> List[Dict[str, Any]]:
    ...
    if isinstance(gt_data, dict):
        for key in ["data", "가입가능조건", "rows", "items", "definitions"]:
            if key in gt_data and isinstance(gt_data[key], list):
                return gt_data[key]
        raise ValueError(f"GT JSON에서 list 형태 rows를 찾지 못함: {gt_path}")

    raise ValueError(f"GT JSON 구조 이상: {gt_path}")


def load_pred_rows(pred_path: Path) -> List[Dict[str, Any]]:
    ...
    if isinstance(pred_data, dict):
        if (
            "final_data" in pred_data
            and isinstance(pred_data["final_data"], dict)
            and "definitions" in pred_data["final_data"]
            and isinstance(pred_data["final_data"]["definitions"], list)
        ):
            return pred_data["final_data"]["definitions"]
        if "definitions" in pred_data and isinstance(pred_data["definitions"], list):
            return pred_data["definitions"]

        for key in ["data", "rows", "items"]:
            if key in pred_data and isinstance(pred_data[key], list):
                return pred_data[key]

        raise ValueError(f"Pred JSON에서 rows/definitions list를 찾지 못함: {pred_path}")

    raise ValueError(f"Pred JSON 구조 이상: {pred_path}")


def find_unique_file_by_name(root_dir: Path, filename: str) -> Path:
    matches = [p for p in root_dir.rglob(filename) if p.is_file()]
    if not matches:
        raise FileNotFoundError(f"{root_dir} 아래에서 파일을 찾지 못함: {filename}")
    if len(matches) > 1:
        candidates = "\n".join(str(p) for p in matches[:20])
        raise ValueError(f"{root_dir} 아래에서 파일명이 중복됨: {filename}\n{candidates}")
    return matches[0]


def find_unique_file_by_doc_id(root_dir: Path, doc_id: str) -> Path:
    doc_id = str(doc_id).strip().lstrip("0") or "0"
    pattern = re.compile(rf"^(0*{re.escape(doc_id)})\.")
    matches = [
        p for p in root_dir.rglob("*.json")
        if p.is_file() and pattern.match(p.name)
    ]
    if not matches:
        raise FileNotFoundError(f"{root_dir} 아래에서 doc-id 파일을 찾지 못함: {doc_id}")
    if len(matches) > 1:
        candidates = "\n".join(str(p) for p in sorted(matches)[:20])
        raise ValueError(f"{root_dir} 아래에서 doc-id가 중복됨: {doc_id}\n{candidates}")
    return matches[0]


def main() -> int:
    parser = argparse.ArgumentParser(description="단일 문서 GT/Pred F1 평가")
    parser.add_argument("--gt-dir", type=Path, default=Path("GT"), help="GT 디렉토리(재귀 탐색)")
    parser.add_argument(
        "--pred-dir",
        type=Path,
        default=Path("추가데이터_pred"),
        help="Pred 디렉토리(재귀 탐색)",
    )
    group = parser.add_mutually_exclusive_group(required=False)
    group.add_argument("--name", type=str, help="GT/Pred에서 동일한 파일명으로 매칭")
    group.add_argument("--doc-id", type=str, help="파일명 접두어 doc-id(예: 056)로 매칭")
    parser.add_argument("--gt-file", type=Path, help="GT 파일 직접 지정(우선순위 높음)")
    parser.add_argument("--pred-file", type=Path, help="Pred 파일 직접 지정(우선순위 높음)")
    ...
    if args.gt_file and args.pred_file:
        gt_path = args.gt_file
        pred_path = args.pred_file
    elif args.name:
        gt_path = find_unique_file_by_name(gt_dir, args.name)
        pred_path = find_unique_file_by_name(pred_dir, args.name)
    elif args.doc_id:
        gt_path = find_unique_file_by_doc_id(gt_dir, args.doc_id)
        pred_path = find_unique_file_by_doc_id(pred_dir, args.doc_id)
    else:
        parser.error("`--gt-file/--pred-file` 또는 `--name` 또는 `--doc-id` 중 하나를 지정하세요.")
        return 2

    gt_rows = load_gt_rows(gt_path)
    pred_rows = load_pred_rows(pred_path)

    cleaned_weights = {k.strip(): v for k, v in weights.items()}
    cleaned_block_primary = [f.strip() for f in block_primary]
    cleaned_block_fallback = [f.strip() for f in block_fallback]

    report = evaluate_records(
        gt_rows=gt_rows,
        pred_rows=pred_rows,
        schema_fields=schema_fields,
        block_fields_primary=cleaned_block_primary,
        block_fields_fallback=cleaned_block_fallback,
        weights=cleaned_weights,
        match_threshold=args.match_threshold,
    )

    metrics = report["record_metrics"]
    print(f"doc: gt={gt_path} pred={pred_path}")
    print(f"f1: {metrics['f1']}  (precision={metrics['precision']}, recall={metrics['recall']})")

    if args.verbose:
        print("tp/fp/fn:", metrics["tp"], metrics["fp"], metrics["fn"])
